...
php/sy/doctrine/db/create: ## Create database (idempotent)
	$(call php_sy_bin_console,doctrine:database:create --if-not-exists $(php_sy_conn_flag) )
...
php/sy/doctrine/db/drop: ## Drop database (idempotent) [requires confirmation]
	$(if $(call mb_user_confirm,Drop database $(if $(php_sy_conn),for) connection '$(php_sy_conn)'?) ,\
		$(call php_sy_bin_console,doctrine:database:drop --if-exists --force $(php_sy_conn_flag))
	,\
...
php/sy/doctrine/schema/validate: ## Validate mapping vs DB
	$(call php_sy_bin_console,doctrine:schema:validate )
...
php/sy/doctrine/schema/create: ## Create tables from mapping (no migrations)
	...
	$(call php_sy_bin_console,doctrine:schema:create )
...
php/sy/doctrine/schema/update: ## Update schema to match mapping (no migrations)
	...
	$(call php_sy_bin_console,doctrine:schema:update $(php_sy_schema_update_flags) --force )

# -------------------- Migrations --------------------
php/sy/doctrine/mig/diff: ## Generate migration from mapping changes
	$(call php_sy_bin_console,doctrine:migrations:diff )

php/sy/doctrine/mig/migrate: ## Run pending migrations
	$(call php_sy_bin_console,doctrine:migrations:migrate $(php_sy_doctrine_migrate_flags) )

php/sy/doctrine/mig/status: ## Show migrations status
	$(call php_sy_bin_console,doctrine:migrations:status )

php/sy/doctrine/mig/list: ## List all migrations
	$(call php_sy_bin_console,doctrine:migrations:list )

php/sy/doctrine/mig/dump: ## Dump current DB schema into a migration (baseline)
	$(call php_sy_bin_console,doctrine:migrations:dump-schema )
...
php/sy/doctrine/fixtures/load: ## Load fixtures (purges DB unless --append)
	...
	$(call php_sy_bin_console,doctrine:fixtures:load $(php_sy_fixtures_flags) $(php_sy_doctrine_migrate_flags) )

# -------------------- Common flows --------------------
php/sy/doctrine/dev/rebuild: php/sy/doctrine/db/reset .WAIT php/sy/doctrine/mig/migrate .WAIT php/sy/doctrine/fixtures/load
php/sy/doctrine/dev/rebuild: ## Reset DB, migrate, load fixtures (dev seed)

php/sy/doctrine/test/reset: php_sy_env := test
php/sy/doctrine/test/reset: php/sy/doctrine/db/reset .WAIT php/sy/doctrine/mig/migrate
